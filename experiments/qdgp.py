...
def MC_fitness(individual, n_steps, num_episodes, gamma, features_kept):
    ...
    for e in range(num_episodes):
        ...
        for t in range(n_steps):
            ...
            elif "BipedalWalker" in  gp_script.ENV.unwrapped.spec.id:#gp_script.ENV.unwrapped.spec.id  == "BipedalWalker-v3":
...
if '__main__' == __name__:
    ...
    parser.add_argument("--path", help="path to save the results", default=os.path.join("experiments", "results", "qdgp"), type=str)
    ...
    pop, log = algo.qdLambda(pop, gp_script.toolbox, grid, args.batch_size, cxpb=args.cxpb, mutpb=args.mutpb, lambda_=args.lambda_, ngen=args.n_gen, stats=mstats, iteration_callback=save_each_generation(dir, modulo=args.save_every), verbose=True)
