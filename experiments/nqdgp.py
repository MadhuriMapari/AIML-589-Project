...
def MC_fitness(individual, n_steps, num_episodes, gamma):
    ...
    total_num_features = [0, ] * len(gp_script.all_features_domain)
    grid_features = []
    rExp = re.compile("([a-zA-Z]+)([0-9]+)")
    ...
    for ind in individual:
        for node in ind:
            if isinstance(node, gp.Terminal) and not isinstance(node, gp.Ephemeral):
                _, index = rExp.match(node.value).groups()
                total_num_features[int(index)] += 1
    grid_features.append(np.count_nonzero(total_num_features))
    ...
    if nb_weights > 20:
        nb_weights = 20
    grid_features.insert(0, nb_weights)

    cpl = team.team_complexity(individual, gp_utils.complexity)
    if cpl >= 100:
        cpl = 100
    grid_features.insert(0, cpl)

    grid_features = np.array(grid_features)

    if s / num_episodes < -500_000.0:
        return -500_000.0, *grid_features
    return s / num_episodes, *grid_features
